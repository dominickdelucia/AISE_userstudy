...
def grab_volume_features(price_df):
    ...
    df['bid_VWA_diff'] = df.apply(lambda x: (x.BID0_PRICE*x.BID0_QTTY + x.BID1_PRICE*x.BID1_QTTY + x.BID2_PRICE*x.BID2_QTTY + x.BID3_PRICE*x.BID3_QTTY + x.BID4_PRICE*x.BID4_QTTY)/x.bid_volume - x.close_price, axis =1)
    df['ask_VWA_diff'] = df.apply(lambda x: (x.ASK0_PRICE*x.ASK0_QTTY + x.ASK1_PRICE*x.ASK1_QTTY + x.ASK2_PRICE*x.ASK2_QTTY + x.ASK3_PRICE*x.ASK3_QTTY + x.ASK4_PRICE*x.ASK4_QTTY)/x.ask_volume - x.close_price, axis =1)
    
    return_cols = ['idx','timestamp','close_price','total_volume','vol_buy','vol_sell','near_spread','far_spread',
...
def augment_volume_features(vol_df, roll_bars):    
    ...
    z_feats = ['close_price','total_volume','vol_buy','vol_sell','near_spread','far_spread','bid_volume','ask_volume','bid_VWA_diff','ask_VWA_diff']
    for col in z_feats:
...
def bar_streak(df, bar_lookback = 30):
    ...
    tmp_p['streak'] = tmp_p.apply(check_across, axis=1)
    tmp_n['streak'] = tmp_n.apply(check_across, axis=1)
    df['pos_streak.Alpha'] = tmp_p.streak
    df['neg_streak.Alpha'] = tmp_n.streak
    cols.append('pos_streak.Alpha')
    cols.append('neg_streak.Alpha')
    return(df.loc[:,cols]) 

def check_across(row):
    row = row.replace([np.nan, np.inf], 0)
    if row[0] == 0:
        return(0)
    for i in range(1,len(row)):
        # figure out where the streak breaks
        if row[i] < row[i-1]:
            return(i)
    return(len(row))
...
def wick_wax_features(df):
    df['wax'] = df.apply(lambda row: abs(row.close_price - row.open_price), axis=1)
    df['wax'] = df.wax.replace(0,.01)
    main_cols = df.columns
    df['up_wick'] = df.apply(lambda row: (row.high_price - max(row.close_price,row.open_price)), axis=1)
    df['down_wick'] = df.apply(lambda row: -1*(row.low_price - min(row.close_price,row.open_price)), axis=1)
    df['up_ww_ratio'] = df.apply(lambda row: row.up_wick/row.wax, axis=1)
    df['down_ww_ratio'] = df.apply(lambda row: row.down_wick/row.wax, axis=1)
    df['wick_diff'] = df.apply(lambda row: row.up_wick - row.down_wick, axis=1)
    cols = df.columns.difference(main_cols)
    for col in cols:
            df = df.rename(columns = {col: "{}.Alpha".format(col)})
    return(df)
...
def many_emas_generation(df, target, window_vec, name_vec):
    main_cols = df.columns
    df = add_many_emas(df, target, window_vec, name_vec)
    df = pctdiff_from_currprice(df)
    cols = df.columns.difference(main_cols)
...
def big_feature_set(raw_df, target, window_vec, name_vec, bar_lookback):
    df = simplify_price_df(raw_df)
    df = many_emas_generation(df, target, window_vec, name_vec)
    df = bar_streak(df, bar_lookback)
    df = wick_wax_features(df)
    return(df)

# going to point to the C++ features in the data config file
# Then this will be the "feature generation" function 
# Basically doing cleaning of the data 
def cppfeat(df, feat_path):
    print("expected path to features file:{}".format(feat_path))
    print("check to make sure this matches above")
    return(df)


def cppfeat_SS(df, feat_path, target):
    print("expected path to features file:{}".format(feat_path))
    print("check to make sure this matches above")
    ww = 30 # 5 min on each side
    df['slope.Alpha'] = df[target].rolling(ww,min_periods=2).apply(lambda x: np.polyfit(range(len(list(x))),list(x),1)[0])
    df['slope.Alpha'] = df['slope.Alpha'].replace([np.nan, np.inf], 0)
    return(df)


function_map = {"cppfeat":cppfeat,
               "cppfeat_SS":cppfeat_SS}
